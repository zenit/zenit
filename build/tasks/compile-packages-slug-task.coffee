...
module.exports = (grunt) ->
  ...
  grunt.registerTask 'compile-packages-slug', 'Add bundled package metadata information to the main package.json file', ->
    appDir = fs.realpathSync(grunt.config.get('zenit.appDir'))
    ...
    for moduleDirectory in fs.listSync(modulesDirectory)
      ...
      continue unless metadata?.engines?.zenit?
      ...
      moduleCache = metadata._zenitModuleCache ? {}
    ...
    metadata._zenitPackages = packages
    metadata._zenitMenu = getMenu(appDir)
    metadata._zenitKeymaps = getKeymaps(appDir)
